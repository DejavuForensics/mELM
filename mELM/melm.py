...
#========================================================================
class melm():
	def main(self,TrainingData_File,TestingData_File,AllData_File,Elm_Type,NumberofHiddenNeurons,ActivationFunction,nSeed,verbose):
		...
		if AllData_File is None:
			...
			#%%%%%%%%%%% Load training dataset
			if verbose: print ('Load training')
			all_data = pd.read_csv(AllData_File, sep=';', decimal=".", low_memory=False, header=None)
			...
			if Elm_Type!=self.REGRESSION:
				samples = np.random.permutation(np.size(all_data,0))
			else:
				samples = range(0, np.size(all_data,0))		#		in time series, order is important
			peace_train = round(0.9*np.size(samples,0))		#% 		90% to train
			samples_train = samples[0: peace_train].tolist()
			samples_test = samples[peace_train+1: np.size(all_data,0)-1].tolist()
			del(samples)
			features = [*range(0, np.size(all_data,1))];
			train_index = np.ix_(samples_train,features);
			train_data = all_data[np.ix_(samples_train,features)];
			
			T = np.transpose(train_data[:,0])
			P = np.transpose(train_data[:,1:np.size(train_data,1)])
			del(train_data)                                   #%   Release raw training data array
			del(samples_train)
			del(train_index)
			
			if verbose: print ('Load testing')
			#%%%%%%%%%%% Load testing dataset
			test_index = np.ix_(samples_test,features);
			test_data = all_data[np.ix_(samples_test,features)];

			TVT = np.transpose(test_data[:,0])
			TVP = np.transpose(test_data[:,1:np.size(test_data,1)])
...
if __name__ == "__main__":
	...
	ff = melm()
	ff.main(opts[0], opts[1], opts[2], opts[3], opts[4], opts[5], opts[6], opts[7])
#========================================================================
